...
def track_pr_propagation(
    ...
    for pr in recent_prs:
        merge_sha = _find_merge_sha(upstream_path, pr)
        if not merge_sha:
            continue

        # For stable and rhoai: find the earliest tag on that branch
        # that contains the commit.  The tag date is a meaningful
        # "arrived at" timestamp (unlike the commit's own date).
